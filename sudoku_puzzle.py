...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        ...
        >>> grid1 += ["B", "A", "D", "C"]
        >>> grid1 += ["D", "C", "B", "*"]
        >>> s = SudokuPuzzle(4, grid1, {"A", "B", "C", "D"})
        >>> s.fail_fast()
        False
        ...
        >>> grid2 += ["B", "*", "*", "A"]
        >>> grid2 += ["D", "C", "*", "B"]
        >>> t = SudokuPuzzle (4, grid2, {"A", "B", "C", "D"})
        >>> t.fail_fast()
        True
        ...
        >>> grid3 += ["B", "*", "*", "A"]
        >>> grid3 += ["D", "C", "*", "B"]
        >>> v = SudokuPuzzle (4, grid3, {"A", "B", "C", "D"})
        >>> v.fail_fast()
        True
        """
        n, symbols, symbol_set = self._n, self._symbols, self._symbol_set

        # check ALL empty spaces
        for i in range(len(symbols)):
            if symbols[i] == "*":
                row = self._row_set(i)
                col = self._column_set(i)
                subsquare = self._subsquare_set(i)
                # If at least one empty position has no valid inputs,
                # return True
                if all([symbol in row or
                        symbol in col or
                        symbol in subsquare
